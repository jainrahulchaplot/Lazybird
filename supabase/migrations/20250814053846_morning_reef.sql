/*
# Initial Lazy Bird Database Schema

## Phase 1: Single User Setup
- All tables configured for single user (user_id = 'me')
- RLS disabled for Phase 1 as specified
- Full text search enabled for leads and snippets
- Comprehensive indexing for performance

## Tables Created:
1. **users** - User profile with headline, locations, salary expectations
2. **resumes** - PDF storage and parsed JSON structure
3. **snippets** - Reusable narrative content (achievements, stories, metrics)
4. **leads** - Job opportunities from various sources (screenshot, URL, manual)
5. **contacts** - Contact information associated with leads
6. **artifacts** - Generated content (cover letters, emails, blurbs)
7. **applications** - Application tracking and stage management
8. **application_contacts** - Many-to-many relationship for application contacts
9. **messages** - Email thread tracking with Gmail integration
10. **files** - File storage metadata for screenshots, resumes, attachments
11. **settings** - User preferences and API credentials
12. **prompt_presets** - Customizable AI prompt templates
13. **followup_rules** - Automated follow-up scheduling rules
14. **followup_templates** - Templates for follow-up messages

## Security & Performance:
- GIN indexes for array fields and full-text search
- BTREE indexes for frequently queried columns
- Foreign key constraints for data integrity
- Encrypted storage consideration for sensitive data
*/
...
CREATE TABLE IF NOT EXISTS leads (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id text NOT NULL DEFAULT 'me',
  source_type text CHECK (source_type IN ('screenshot','url','manual')) NOT NULL,
  source_ref text,
  company text,
  role text,
  location text,
  ...
  nice_to_haves text[],
  keywords text[],
  confidence float4 DEFAULT 0.0,
  status text CHECK (status IN ('new','enriched','ready_for_outreach','archived')) DEFAULT 'new',
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS contacts (
  ...
  linkedin_url text,
  photo_url text,
  source text CHECK (source IN ('parsed','manual')) DEFAULT 'manual',
...
-- Prompt presets table (needed before artifacts)
...
CREATE TABLE IF NOT EXISTS artifacts (
  ...
  type text CHECK (type IN ('cover_letter','email_body','blurb')) NOT NULL,
...
CREATE TABLE IF NOT EXISTS applications (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  lead_id uuid NOT NULL,
  stage text CHECK (stage IN ('identified','applied','warm_intro','interviewing','offer','closed')) DEFAULT 'identified',
...
CREATE TABLE IF NOT EXISTS files (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id text NOT NULL DEFAULT 'me',
  kind text CHECK (kind IN ('screenshot','resume','attachment')) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_applications_next_followup ON applications (next_followup_at);
...
INSERT INTO users (id, name, email) VALUES ('me', 'User', 'user@example.com') ON CONFLICT (id) DO NOTHING;

-- Insert default followup rules
INSERT INTO followup_rules (user_id, schedule, pause_on_reply, workdays_only, timezone) 
VALUES ('me', '[{"offsetDays": 2, "templateId": null}, {"offsetDays": 7, "templateId": null}]', true, true, 'Asia/Kolkata')
ON CONFLICT DO NOTHING;

-- Insert seed prompt presets
INSERT INTO prompt_presets (user_id, name, artifact_type, prompt_template, variables) VALUES
('me', 'Cover Letter - Honest & Standout (PM)', 'cover_letter', 
...
('me', 'Recruiter Email - Short & Human', 'email_body',
...
('me', 'Blurb - JD Portal', 'blurb',
'Write a 2-3 sentence targeted summary for application boxes.
Focus on the most relevant experience for {{role}} at {{company}}.
Use quantified achievements where possible.
Profile: {{profile}}
Resume: {{resume_json}}
Job Requirements: {{must_haves}}',
ARRAY['role', 'company', 'profile', 'resume_json', 'must_haves'])
ON CONFLICT DO NOTHING;

-- Insert seed followup templates
INSERT INTO followup_templates (user_id, name, prompt_template, variables) VALUES
('me', 'Gentle Bump', 
...
Inputs: {{company}}, {{role}}, {{last_email_summary}}',
